/**
 * SHA-256 Benchmark for ts-aot
 * Tests: Uint32Array, Bitwise operations, Loop unrolling, and Integer math.
 */
...
function sha256(message: string): string {
    ...
    const msg = new Uint8Array(nBlocks << 6);
    ...
    view.setUint32((nBlocks << 6) - 4, msgLen * 8);

    const w = new Uint32Array(64);

    for (let b = 0; b < nBlocks; b++) {
        for (let t = 0; t < 16; t++) {
            w[t] = view.getUint32((b << 6) + (t << 2));
        }
