...
void ts_promise_resolve_microtask(void* data) {
    ...
    for (auto& cb : promise->callbacks) {
        ...
        } else if (cb.onFulfilled.type == ValueType::OBJECT_PTR && cb.onFulfilled.ptr_val) {
...
void ts_promise_resolve_internal(TsPromise* promise, TsValue* value) {
    ...
    task->value = value ? *value : TsValue();
...
void ts_async_await(TsValue* promise, AsyncContext* ctx) {
    if (!promise || promise->type != ValueType::OBJECT_PTR || !promise->ptr_val) {
...
void TsPromise::then_async(AsyncContext* asyncCtx) {
    Callback cb;
    cb.onFulfilled = nullptr;
    cb.onRejected = nullptr;
    ...
    if (state != PromiseState::Pending) {
        ...
        ts_queue_microtask([](void* data) {
            auto t = static_cast<CallbackTask*>(data);
            ts_async_resume(t->cb.asyncCtx, &t->promise->value);
        }, task);
    } else {
        callbacks.push_back(cb);
    }
