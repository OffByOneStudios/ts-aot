...
ast::ExprPtr Parser::parsePrimaryExpression() {
    ...
    switch (tok.kind) {
        ...
        case TokenKind::NoSubstitutionTemplate: {
            ...
            node->rawHead = text;  // un-escaped source (for String.raw)
            return node;
        }
...
ast::ExprPtr Parser::parseTemplateLiteral(bool isUntagged) {
    ...
    node->rawHead = headText;  // un-escaped source (for String.raw)
    advance();
    ...
    while (true) {
        ...
        if (contTok.kind == TokenKind::TemplateTail) {
            ...
            span.rawLiteral = litText;  // un-escaped source (for String.raw)
...
ast::ExprPtr Parser::parseTaggedTemplate(ast::ExprPtr tag) {
    ...
    if (check(TokenKind::NoSubstitutionTemplate)) {
        ...
        tmpl->rawHead = text;  // un-escaped source (for String.raw)
        advance();
...
ast::ExprPtr Parser::parseClassExpression() {
    ...
    // extends + implements + body — shared with parseClassDeclaration via
    // parseClassHeritageClause/parseClassBodyInto so the ECMA-262 15.7.1 early
    // errors (constructor count, private get/set static agreement, etc.) apply
    // to class EXPRESSIONS too.
