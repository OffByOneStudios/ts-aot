    ...
    if (sym) {
        ...
        if (lastType && lastType->kind == TypeKind::Function) {
            ...
            if (hasValidParams) {
                ...
                auto importRes = resolveImportSourcePath(node->name);
                functionUsages[node->name].push_back({funcType->paramTypes, {}, modPath, importRes.modulePath, importRes.originalName});
            }
        }
    } else {
